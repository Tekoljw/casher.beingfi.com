  ...
  useEffect(() => {
    ...
    class App {
      ...
      tick() {
        ...
        this.timeOffset += delta * this.speedUp;
        let time = this.clock.elapsedTime + this.timeOffset;

        this.camera.fov += (this.fovTarget - this.camera.fov) * 0.05;
        this.speedUp += (this.speedUpTarget - this.speedUp) * 0.01;
        ...
        let distortion = this.options.distortion.getJS(0.025, time);
        this.camera.lookAt(
          new THREE.Vector3(
    ...
    class CarLights {
      ...
      constructor(webgl: App, options: any, colors: number[], speed: number[], fade: THREE.Vector2) {
        this.webgl = webgl;
        this.options = options;
        this.colors = colors;
        this.speed = speed;
      ...
      init() {
        const options = this.options;
        let curves = new THREE.TubeGeometry(
        ...
        let instanced = new THREE.InstancedBufferGeometry().copy(curves);
        ...
        if (Array.isArray(colors)) {
          colors = colors.map((c) => new THREE.Color(c)) as any;
        } else {
        ...
        for (let i = 0; i < options.lightPairsPerRoadWay; i++) {
          ...
          let offsetX = random(options.carShiftX) * options.roadWidth;
          ...
          let color = pickRandom(colors as any) as THREE.Color;
          aColor.push(color.r);
          aColor.push(color.g);
          aColor.push(color.b);
